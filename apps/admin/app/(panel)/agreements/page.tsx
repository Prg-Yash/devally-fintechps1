        ...
        {selectedAgreement ? (
          <>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              <p><strong>Amount:</strong> {formatAmount(selectedAgreement.amount)} {selectedAgreement.currency}</p>
              <p><strong>Created:</strong> {formatDate(selectedAgreement.createdAt)}</p>
              <p><strong>Creator:</strong> {selectedAgreement.creator.name} ({selectedAgreement.creator.email})</p>
              <p><strong>Receiver:</strong> {selectedAgreement.receiver.name} ({selectedAgreement.receiver.email})</p>
            </div>

            <div className="rounded-xl border border-[#d9d0bf] bg-[#fefcf6] p-4">
              <h4 className="mb-2 mt-0">Milestones</h4>
              <p>Total milestones: {selectedAgreement.milestones.length}</p>
              <p>Linked tickets: {selectedAgreement._count.tickets}</p>
            </div>
            ...
            <details className="rounded-xl border border-dashed border-[#d9d0bf] bg-[#fcfaf5] p-3">
              ...
              <pre className="mt-3 max-h-[340px] overflow-auto whitespace-pre-wrap break-words text-xs">
                {JSON.stringify(selectedAgreement, null, 2)}
              </pre>
            </details>
          </>
        ) : null}
